...
function crearProcesos(id){
    ...
    const tiempo = randomNumber(6, 18);
    const operacion = generarOperacion();
    
    const errores = validar(operacion);

    if(isEmpty(errores)){
        const proceso =  new Proceso(id, nombre, tiempo, operacion,tiempo);
...
async function ejecutar() {
    ...
    while (lotes.length) {
        ...
        while (loteActual.length) {
            ...
            if (proceso.tiempoEjecucion > 0) {
                PROCESS_TIMER.repeat(proceso.tiempoEstimado - proceso.tiempoEjecucion);
            }

            actualizarPoceso();

            try {
                await procesoEjecucion(abortSignal, proceso);
                ProcesosTerminados();
                tiempoEjecucion += proceso.tiempoEjecucion;
            } catch (err) {
                console.warn(err);
                continue;
            } finally {
    ...
    actualizarPoceso(); // Llama a actualizarPoceso cuando no hay más procesos en ejecución
    console.log('termine');
    GLOBAL_TIMER.destroy();
    PROCESS_TIMER.destroy();
}
...
async function procesoEjecucion(signal, proceso) {
    const tiempos = document.querySelectorAll('#tiempo');
    let i = proceso.tiempoEjecucion; // Inicializa con el valor de tiempoEjecucion del proceso
    return new Promise((resolve, reject) => {
        PROCESS_TIMER
            .action(t => {
                proceso.tiempoEjecucion = i; // Actualiza el tiempoEjecucion del proceso
                tiempos[0].innerHTML = `Tiempo Transcurrido: <span>${proceso.tiempoEjecucion} segundos</span>`;
                tiempos[1].innerHTML = `Tiempo Restante: <span>${proceso.tiempoEstimado - proceso.tiempoEjecucion} segundos</span>`;
                ++i;
            })
...
function actualizarPoceso() {
    ...
    if (loteActual.length > 0) {
        ...
        // No quedan procesos en loteActual, imprimir "Actualizar proceso" con valores en NULL
        divProceso.innerHTML = `
            <p>ID: <span>NULL</span></p>
            <p>Nombre: <span>NULL</span></p>
